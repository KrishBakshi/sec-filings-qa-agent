...
collection = chroma_client.get_or_create_collection(name="sec_filings_test")
...
filepaths = glob.glob(os.path.join(MARKDOWN_DIR, "*.md"))[:3]
print(f"Found {len(filepaths)} markdown files to test")
...
if os.path.exists(CHROMA_DB_DIR):
    print(f"ChromaDB directory created: {CHROMA_DB_DIR}")
    print("Directory contents:")
    for root, dirs, files in os.walk(CHROMA_DB_DIR):
        level = root.replace(CHROMA_DB_DIR, '').count(os.sep)
        indent = ' ' * 2 * level
